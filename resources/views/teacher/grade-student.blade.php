<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            📝 Review Student: {{ $student->user->name }} - {{ $section->name }}
        </h2>
    </x-slot>
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            ...
                            <p class="text-sm text-gray-600">Student: {{ $student->user->name }}</p>
                        </div>
                    ...
                    @if($allAttempts->count() > 1)
                        <div class="mb-6 flex gap-2">
                            ...
                            @foreach($allAttempts as $num)
                                <a href="{{ route('teacher.grade-student', [$student->studentID, $section->id, 'attempt' => $num]) }}"
                    ...
                    <div class="space-y-4">
                        @foreach($assessments as $assessment)
                            <div
                                class="p-4 rounded border @if($assessment->was_correct) bg-green-50 border-green-200 @else bg-red-50 border-red-200 @endif">
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        @if($assessment->image)
                                            <img src="{{ asset('storage/' . $assessment->image) }}" alt="Question"
                                        ...
                                        <div class="space-y-2">
                                            <div class="flex items-center p-2 rounded 
                                                ...
                                                @if($assessment->student_answer === 'A')
                                                    <span class="ml-2 text-sm font-semibold">← Student Answer</span>
                                                @endif
                                                @if($assessment->correct_answer === 'A')
                                            ...
                                            <div class="flex items-center p-2 rounded 
                                                ...
                                                @if($assessment->student_answer === 'B')
                                                    <span class="ml-2 text-sm font-semibold">← Student Answer</span>
                                                @endif
                                                @if($assessment->correct_answer === 'B')
                                                    <span class="ml-2 text-green-600 font-bold">✓</span>
                                                @endif
                                            </div>
                                            ...
                                            <div class="flex items-center p-2 rounded 
                                                ...
                                                @if($assessment->student_answer === 'C')
                                                    <span class="ml-2 text-sm font-semibold">← Student Answer</span>
                                                @endif
                                                @if($assessment->correct_answer === 'C')
                                                    <span class="ml-2 text-green-600 font-bold">✓</span>
                                                @endif
                                            </div>
                                        </div>
                    ...
                    <div class="mt-6">
                        <a href="{{ route('teacher.grading', ['course_id' => $courseId ?? '', 'section_id' => $sectionId ?? '']) }}"
